...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      let query = db.from("deals").select("*, tours(title, image_url, price, discount_price)");
    ...
    if (req.method === "PUT") {
      ...
      const { data, error } = await db.from("deals").update(updates).eq("id", dealId).select().single();
      if (error) throw error;
      return new Response(JSON.stringify({ data }), {
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    ...
    if (req.method === "DELETE") {
      const body = await req.json();
      const { error } = await db.from("deals").delete().eq("id", body.id);
      if (error) throw error;
